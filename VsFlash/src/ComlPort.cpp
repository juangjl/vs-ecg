...
{	
  ...
  unsigned long 	dwErr = 0;
	unsigned long 	dwExitCode = COMPORT_THREAD_EXIT_CODE_EVENT_OK;  
...
JBOOL ComPortFlush(JINT iReadLen)
{
	JBOOL bRet = FALSE;
	DevCPType *pDevCP = &DevCP0;
	JINT iTimeOut = 10;
	JBYTE bReadData[1024];
	
	bRet = ComPortRead(pDevCP, (JBYTE *)&bReadData[0],  iReadLen, iTimeOut);
	if(bRet == FALSE)
	{
		return bRet;
	}
