...
JVOID PacketAckInit(PacketAckType *pPacketAck)
{
	JINT i;
...
JINT PacketSendBootCode(PacketType * pPacketOut, PacketAckType * pPacketIn, int iTimeout)
{
  JINT errNo = NO_ERR;
	static JWORD wSend = 0;
	JBOOL bRet = FALSE;
  ...
  iWriteLen = pPacketOut->wMOSILen + PACKET_ACK_HEADER_SIZE + PACKET_CMD_BDATA_SIZE;
  iReadLen  = pPacketOut->wMISOLen + PACKET_ACK_HEADER_SIZE;
	bRet 		  = SERIAL_PORT_TRANSFER((JBYTE *)pPacketOut, (JBYTE *)pPacketIn,  iWriteLen, iReadLen, iTimeout);
  if(bRet == FALSE)
	{
